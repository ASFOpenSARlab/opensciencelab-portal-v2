...
def get_event(path="/", method="GET", cookies=None, headers=None, qparams=None, body=None):
...
class TestProfilePages:
    ...
    def test_profile_loading(self, lambda_context: LambdaContext, monkeypatch, fake_auth):
        ...
        assert ret["body"].find("data-id-to-show=\"user_or_pi_nasa_email_div\"\nselected") != -1
    ...
    def test_process_profile_form(self, monkeypatch):
        ...
        def validate_profile_true(query_dict:dict) -> tuple[bool, dict[str, str]]:
            return True, {}
        monkeypatch.setattr("portal.profile.validate_profile_dict", validate_profile_true)
        ...
        monkeypatch.setattr("portal.profile.validate_profile_dict", validate_profile_false)
    ...
    def test_profile_user_filled(self, monkeypatch, lambda_context: LambdaContext, fake_auth):
        ...
        ## Test correct filling
        def process_profile_form_correct(*args, **kwargs) -> tuple[bool, dict[str, str]]:
            return True, {
            'country_of_residence': 'US',
            'is_affiliated_with_nasa': 'no',
            'is_affiliated_with_us_gov_research': 'no',
        ...
        monkeypatch.setattr("portal.profile.process_profile_form", process_profile_form_correct)
        query_dict={
            'country_of_residence': 'US',
            'is_affiliated_with_nasa': 'no',
            'user_or_pi_nasa_email': 'default',
        ...
        event = get_event(path=f"/portal/profile/{user}", method="POST", body=request_body, cookies=fake_auth)
        ret = main.lambda_handler(event, lambda_context)
        ...
        monkeypatch.setattr("portal.profile.process_profile_form", process_profile_form_wrong)
        query_dict={
            'country_of_residence': 'US',
            'is_affiliated_with_nasa': 'no',
            'user_or_pi_nasa_email': 'default',
        ...
        assert ret["statusCode"] == 302
        assert ret["body"] == f"\"{{\'Redirect to /portal/profile/test_user?{expected_query_string}\'}}\""
