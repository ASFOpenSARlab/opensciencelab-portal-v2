...
class TestAuthSession:
    ...
    def test_session_user_saved(self, lambda_context, monkeypatch, helpers, fake_auth, mocked_requests_post):
    ...
    def test_session_no_cookie(self, lambda_context, monkeypatch, helpers, mocked_requests_post):
        # Create FakeUser instance to be monkeypatched in and inspected after modified
        user = helpers.FakeUser()
        monkeypatch.setattr("main.User", lambda *args, **kwargs: user)
        monkeypatch.setattr("util.auth.User", lambda *args, **kwargs: user)
        ...
        assert current_session.user is None, "User should not be set before handler"
        main.lambda_handler(event, lambda_context)
        assert current_session.user is None, "User should still be None without a auth cookie"

    def test_session_user_overridden(self, lambda_context, monkeypatch, helpers, mocked_requests_post):
        # Create FakeUser instance to be monkeypatched in and inspected after modified
        user = helpers.FakeUser()
        monkeypatch.setattr("main.User", lambda *args, **kwargs: user)
        monkeypatch.setattr("util.auth.User", lambda *args, **kwargs: user)
